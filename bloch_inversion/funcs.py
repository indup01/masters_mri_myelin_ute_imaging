...
def setup_M_plots(M,dB0):
    ...
    M_padded[:,1,:]=M
    Mpc1, = ax1.plot (M[0,:],M[2,:],'k', lw=2, zorder=2)
    Mpc2, = ax2.plot (M[0,:],M[1,:],'k', lw=2, zorder=2)
    Mxy1, = ax3.plot (dB0,np.abs(M[0,:]+1j*M[1,:]),'k', lw=2, zorder=2)


    for ln_no in range(np.size(dB0)):
        Mp1, = ax1.plot (M_padded[0,:,ln_no],M_padded[2,:,ln_no], c=cm.rainbow((ln_no+0.01)/np.size(dB0)), lw=0.4, zorder=3)
    ...
    M_RF, = ax1.plot (np.array([0,0]),np.array([0,0]),'k',lw=8)
    ...
    txt_RF = ax1.text(0,0, "", color='k', weight='bold')
...
def update_M_plots(dB0,M,ax1,ax2,ax3,Mpc1,Mpc2,Mxy1):
    ...
    for dB0_idx in range(np.size(dB0)):
        ...
        ax_temp.set_data(np.array([0,M[0,dB0_idx]]),np.array([0,M[2,dB0_idx]]))
