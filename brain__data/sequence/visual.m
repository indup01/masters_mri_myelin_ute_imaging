...
sliceNumber = 35;  % Example: middle slice is 48 in a 96x96x96 volume (can be modified)
...
clear
% Load the .mat file
data = load('Z:\home\mri_fyp\brain\sequence\short_no_noise.mat'); % Replace with your actual file name
img = data.one_no_noise; % Access the 3D image

% Set up video writer
video_name = 'slice_video.avi';
v = VideoWriter(video_name, 'Motion JPEG AVI'); % Create video writer
v.FrameRate = 10; % Adjust frame rate as needed
open(v);

% Create figure
fig = figure;
colormap(gray); % Set colormap for grayscale images

for i = 1:size(img, 3) % Loop through slices along the 3rd axis
    imagesc(img(:,:,i)); % Display the current slice
    axis equal off; % Keep aspect ratio and remove axes
    title(sprintf('Slice %d', i)); % Display slice number
