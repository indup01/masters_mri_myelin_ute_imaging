...
%% Select the same ROI from t2_star
masked_t2_star = t2_star(:, :, sliceNumber); % Get the corresponding t2_star slice
masked_t2_star(~mask) = 0; % Apply the mask (zero out values outside ROI)
...
image_data = data.one_no_noise; % Adjust this variable name if different

% Extract the 25th slice along the 3rd axis
slice_25 = image_data(:, :, sliceNumber);

% Display the image
figure;
imshow(slice_25, []);
title('Draw a rectangle for the Signal Region');

% User selects signal region using a rectangle
h = imrect;
pos_signal = round(getPosition(h)); % Get rectangle position
signal_region = imcrop(slice_25, pos_signal);

% Compute mean signal intensity
signal_mean = mean(signal_region(:));

% Prompt user to select noise region
title('Draw a rectangle for the Noise Region');
h = imrect;
pos_noise = round(getPosition(h));
noise_region = imcrop(slice_25, pos_noise);
...
% Display the result
fprintf('Computed SNR: %.2f\n', SNR);

%%
% Display the image
figure;
imshow(slice_25, []);
title('Draw a rectangle for Region 1 (Tissue of Interest)');

% User selects first region (S1)
h = imrect;
wait(h);
pos_region1 = round(getPosition(h));
region1 = imcrop(slice_25, pos_region1);
S1 = mean(region1(:));

% Select second region (S2 - background/reference tissue)
title('Draw a rectangle for Region 2 (Background or Reference Tissue)');
h = imrect;
wait(h);
pos_region2 = round(getPosition(h));
region2 = imcrop(slice_25, pos_region2);
S2 = mean(region2(:));

% Select noise region for standard deviation
title('Draw a rectangle for Noise Region');
h = imrect;
wait(h);
pos_noise = round(getPosition(h));
noise_region = imcrop(slice_25, pos_noise);
sigma = std(noise_region(:));

% Compute CNR
CNR = abs(S1 - S2) / sigma;

% Display the result
fprintf('Computed CNR: %.2f\n', CNR);
