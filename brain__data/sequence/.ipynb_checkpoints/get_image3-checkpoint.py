...
for x in range (23,33):
    ...
    # Load the .mat file
    coil_data = scipy.io.loadmat(f'coil_data/TE1750us_TI350ms/coil_{x}.mat')
    ...
    #plt.show()
    data_f = "images"
    subfolder = "TE1750us_TI350ms"  # Replace with your desired subfolder name
    
    # Create the main folder and the subfolder if they don't exist
    os.makedirs(os.path.join(data_f, subfolder), exist_ok=True)

    reshaped = np.reshape(image_sharp_numpy, (-1, fov))
    #reshaped = image_sharp_numpy.reshape(-1, 3)
    file_path = os.path.join(data_f, subfolder, f'imagesharp{x}.txt')
